...
async def register_student_to_supabase(user):
    try:
        await asyncio.to_thread(lambda: supabase.table("students").upsert({"telegram_id": user.id, "username": user.username, "first_name": user.first_name}, on_conflict="telegram_id").execute())
    except Exception as e:
...
# ... (بقية الدوال المساعدة السابقة get_categories_keyboard, get_exams_keyboard, send_secured_document) ...
def get_categories_keyboard(subject_name):
    keyboard = [["📝 الملخصات الذهنية", "📖 الكتاب المدرسي"], ["💡 ملاحظات تذكيرية", "📒 النوط الشاملة"], ["📂 أسئلة السنوات السابقة"]]
    if "الإسلامية" in subject_name or "🕋" in subject_name: keyboard.insert(2, ["🔊 الأحاديث الشريفة", "🔊 الآيات القرآنية"])
...
async def send_secured_document(context, chat_id, f_id, f_url, caption_text):
    try:
        if f_id: await context.bot.send_document(chat_id=chat_id, document=f_id, caption=caption_text, parse_mode="HTML")
        elif f_url: await context.bot.send_document(chat_id=chat_id, document=f_url, caption=caption_text, parse_mode="HTML")
        return True
    except: return False
...
async def handle_bot_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ... (ضع باقي المنطق الخاص بك هنا - التعامل مع القراء والملفات) ...
    # (ملاحظة: لقد قمت باختصار المنطق هنا لتناسب الرد، لكنه مطابق لمنطقك السابق تماماً)
    
    await update.message.reply_text("ℹ️ استخدم القائمة السفلية.", reply_markup=get_main_keyboard())

def main():
    application = Application.builder().token(BOT_TOKEN).build()
    
    # تفعيل نظام الإعلانات التلقائي (يفحص كل ساعة)
